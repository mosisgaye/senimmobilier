...
export default function ImageUpload({
  ...
  const handleFiles = async (fileList: FileList) => {
    ...
    if (files.length + fileList.length > maxFiles) {
      setGlobalError(`Vous ne pouvez t�l�charger que ${maxFiles} fichier(s) maximum`)
      return
    }
        ...
        >
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <div className="text-xs text-gray-500">
              {files.length} / {maxFiles} fichier(s) t�l�charg�(s)
            </div>
          </div>
        </motion.div>
      )}
        ...
        <div className="flex items-center justify-between text-sm text-gray-600 pt-2 border-t">
          <div className="flex items-center gap-4">
            <span className="flex items-center gap-1">
              ...
              {files.filter(f => f.status === 'success').length} r�ussi(s)
            </span>
            {files.some(f => f.status === 'error') && (
              <span className="flex items-center gap-1">
                <AlertCircle className="h-4 w-4 text-red-600" />
                {files.filter(f => f.status === 'error').length} �chou�(s)
              </span>
            )}
